    ...
    <section className="grid gap-10 py-12 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
      ...
      <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-5 shadow-2xl shadow-black/30 backdrop-blur-xl">
        <div className="rounded-[1.5rem] border border-white/10 bg-black/30 p-6">
          <p className="mb-5 text-sm uppercase tracking-[0.3em] text-white/40">Snapshot</p>
          <div className="grid gap-4">
            <SnapshotRow label="Current focus" value="Tech, product, strategy, AI" />
            <SnapshotRow label="Strength" value="Turning ambiguity into execution" />
            <SnapshotRow label="Profile" value="Engineer + founder + operator" />
          </div>
        </div>
      </div>
    </section>
...
function SnapshotRow({ label, value }: { label: string; value: string }) {
  return <div className="flex items-start justify-between gap-6 border-b border-white/10 pb-4 last:border-b-0 last:pb-0"><p className="text-sm text-white/40">{label}</p><p className="max-w-[220px] text-right text-sm font-medium text-white/85">{value}</p></div>;
}
